...
echo vbd_path: $vbd_device_path
echo kpartx -av ${vbd_device_path} 
...
block=${vbd_device##*/}
#echo $block
#cnt=$(ls /dev/mapper | grep "$block" | wc -l)
#echo $cnt
#i=1
#while((i<=cnt));do
#mkdir "${mount_dir}"$i 2>/dev/null
#mount -t ntfs -o rw /dev/mapper/"${block}"$i  "$mount_dir"$i
#i=$((i+1))
#done
#mount /dev/mapper/"${block}"2  "$mount_dir"
#mount -t ntfs /dev/dm-3 "$mount_dir"
mount -t ntfs -o rw /dev/mapper/"${block}"2  "$mount_dir"
